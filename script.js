...
async function fetchPopularMovies() {
    try {
        ...
        displayMovies(data.results); // Llama a displayMovies con la lista de películas populares
    } catch (error) {
        console.error('Error fetching popular movies:', error);
        alert('Error al cargar películas populares. Intenta de nuevo más tarde.');
    }
}
...
async function showMovieDetails(movieId) {
    try {
        ...
        alert('Error al cargar los detalles de la película. Intenta de nuevo más tarde.');
    }
}
...
searchButton.addEventListener('click', async () => {
    ...
    if (query) {
        try {
            ...
            const data = await response.json();
            displayMovies(data.results); // Muestra las películas encontradas
        } catch (error) {
            console.error('Error searching movies:', error);
            alert('Error al buscar películas. Intenta de nuevo más tarde.');
        }
    }
});
